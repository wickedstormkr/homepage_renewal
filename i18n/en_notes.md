...
3. 한국어로만 남는 것: 파이프라인 영상(`media/pipeline-loop-en.mp4`가 없으면 국문 영상), 제품 화면 캡처, 레퍼런스 로고 이미지, 기업 홍보 영상, 뉴스 카드(board.js가 data/posts.json의 국문 글로 다시 그림), 연결된 기사·개인정보처리방침 페이지. 영문 문구는 이것들이 영어라고 주장하지 않는다.
