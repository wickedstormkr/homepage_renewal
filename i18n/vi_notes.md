...
4. **한국어로 남는 것**: 파이프라인 영상(`media/pipeline-loop-vi.mp4`가 없으면 국문 영상), 제품 화면 캡처, 레퍼런스 로고 이미지, 기업 홍보 영상, 뉴스 카드(board.js가 국문 posts.json으로 다시 그린다), 연결된 기사·개인정보처리방침 페이지. 베트남어 문구는 이것들이 베트남어라고 말하지 않는다. "Xem bài viết"(기사 보기)와 화면 주석에 "(tiếng Hàn)"을 붙일지는 선택 사항이다. 국문에 없는 말이라 넣지 않았다.
