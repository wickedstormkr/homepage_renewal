...
test('sitemap includes internal articles and static pages, excludes external posts', () => {
  ...
  assert.match(sitemap, /https:\/\/wickedstorm\.kr\/news\.html/);
  // 기존 sitemap의 정적 항목(privacy 포함)은 재생성 후에도 유지한다.
  assert.match(sitemap, /https:\/\/wickedstorm\.kr\/privacy\.html/);
  // 외부 링크 글은 정적 아티클 URL을 만들지 않는다.
