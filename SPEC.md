...
빌드 스텝 없음(S3 그대로 배포). 스택: HTML + CSS + vanilla JS + GSAP/ScrollTrigger(로컬 vendor).

## 0. 절대 원칙 (성능 헌법 — 위반 시 렉 재발)

이전 프로토타입에서 레티나 Mac 렉의 원인이 "상시 rAF로 매 프레임 갱신되는 풀스크린 캔버스"였음이 측정으로 확인됨. 따라서:

1. **자유 실행(free-running) rAF 루프 금지.** 상시 rAF 루프는 만들지 않는다.
...
6. `prefers-reduced-motion: reduce`: 핀/스크럽 미생성, 인트로/티커 정지(화면 폭에 따라 정적 2/3/5행),
...
- 폰트: 데스크톱 한글은 `Pretendard Variable` v1.3.9 가변 다이나믹 서브셋(로컬 vendoring),
  모바일 한글은 운영체제 UI 폰트, 영문/숫자는 `Sora` Latin 가변 폰트(로컬 woff2)를 쓴다.
...
전역 스크롤은 네이티브 동작을 사용한다. `matchMedia('(min-width: 961px)')`에서만
ScrollTrigger 핀/스크럽을 활성화하고 모바일은 reveal만 사용한다. reduced-motion 시 전부 비활성.
...
- `.hero`를 pin: start 'top top', end '+=55%', scrub 0.6, anticipatePin 1.
- 진행도 p(0→1)가 구동하는 것:
...
.capture
  ...
  .stream-viewport   ← 화면 폭에 따라 2/3/5행 높이 예약, overflow:hidden,
...
```
동작(push, 가시 상태에서만 이어지는 setTimeout):
...
2. 행 수가 현재 화면의 노출 행 수(모바일 2, 태블릿 3, 데스크톱 5)를 초과하면 **FLIP 방식으로 트랙을 밀어올린다**:
...
5. 일시정지 조건: 사용자의 일시정지 버튼, `.hero` 뷰포트 밖(IO), `document.hidden`.
   사용자가 멈춘 상태는 뷰포트·탭 복귀 후에도 유지한다.
6. Statement 데이터: 동사-객체-결과가 논리적으로 짝지어진 이벤트 풀(프로토의 events 배열 재사용).
7. reduced-motion: 화면 폭에 맞는 정적 2/3/5행 렌더, 타이머 없음.

## 6. 헤더/내비/폼 (운영 품질)

- 헤더: 스크롤 방향과 관계없이 고정 노출 + 20px 이후 불투명 배경. 주 메뉴는 Product / References / Standards / Company / News / 도입 문의 순서다.
- 모바일(≤960px): 햄버거 → 풀폭 드로어(링크 탭 시 닫힘, aria-expanded, ESC 닫힘,
  포커스 순환·복귀, 데스크톱 전환 시 자동 닫힘, 열림 중 body 스크롤 잠금).
...
- [ ] 상시 rAF 0건(코드 검색으로 확인 가능해야 함), backdrop-filter 0건
...
2. **티커**: 정적 5행(무JS/reduced-motion 완전 노출). JS 모드는 초기 행을 보존하고 화면 폭에 따라
   2/3/5행을 유지하며 2400ms 간격으로 슬라이드한다. viewport 높이는 JS가 계산하고 resize 때 재계산한다.
...
5. **영상 루프 2개** (`media/`, 총 ~260KB):
   ...
   - IO 25%에서 play/pause, visibilitychange pause, play() reject는 catch. controls는 항상 제공하고,
     사용자가 멈춘 상태는 자동 재생으로 덮어쓰지 않는다. REDUCE 시 autoplay하지 않는다.
...
6. **이미지 비율 규칙 강화**: `.frame img`에 height:auto 필수. 모든 img의 width/height 속성은 실제
   픽셀 비율과 일치(sips로 측정). 제품·GROWA·특허 이미지는 크롭하지 않고 전체를 표시한다.
...
### DoD 추가 항목
- [x] 티커: 화면별 2/3/5행을 안정적으로 예약하고 이후 슬라이드 점프 없음
...
- [x] 고정 배치 img 선언 비율 = 실측 비율, 생성 상세 이미지는 contain 표시와 CLS 0 검증
